...
function Article() {
  ...
  const deleteArticle = async() => {
    ...
    if(res.data.message==='article deleted'){
      ...
      navigate(`/author-profile/article/${art.articleId}`, { state: res.data.article})
    }
  };
  ...
  const saveModifiedArticle = async (editedArticle) => {
    //merging two articles (if any changes it will get updated)
    ...
    let res = await axiosWithToken.put(
      "http://localhost:4000/author-api/article", modifiedArticle
    );
    // console.log(res);
